        ...
        <div className="pb-12 md:pb-20">
          ...
          <div className="relative">
            ...
            {items.map((item, index) => (
              <img key={index} className="absolute rounded-full z-10 animate-float" style={item.style} src={require(`../images//user-${index + 1}.jpg`).default} width={item.size} height={item.size} alt={`User ${index + 1}`} onMouseEnter={() => { setActive(index); setdescriptionOn(true); }} onMouseLeave={() => setdescriptionOn(false)} />
            ))}

            {/* description box */}
            <div className={`opacity-0 transition duration-150 ease-in-out absolute top-0 left-1/2 transform -translate-x-1/2 -mt-6 max-w-xs w-full p-3 bg-white dark:bg-gray-800 text-center shadow-2xl z-20 pointer-events-none ${descriptionOn && 'opacity-100'}`}>
